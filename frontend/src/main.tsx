...
function App() {
  ...
  async function scan() { setScanning(true); setNotice('Восстанавливаем недоимпортированные записи…'); try { const result = await api<{ queued: number; skipped: number; conflicts: number }>('/api/library/recovery', { method: 'POST' }); setNotice(`В очередь: ${result.queued}; пропущено: ${result.skipped}; конфликтов destination: ${result.conflicts}`); await loadLibrary() } catch (error) { setNotice(error instanceof Error ? error.message : 'Восстановление не удалось') } finally { setScanning(false) } }
  async function retryCurrentSource() { if (!recordId || !sourceId) return; setReprocessing(true); try { const result = await api<{ queued: boolean; kind: string | null }>(`/api/library/records/${recordId}/sources/${sourceId}/reprocess`, { method: 'POST' }); setNotice(result.queued ? `Трек поставлен в очередь: ${result.kind ?? 'обработка'}` : 'Трек уже обработан или находится в очереди'); await loadLibrary(); const loaded = await api<Detail>(`/api/library/records/${recordId}`); setDetail(loaded) } catch (error) { const message = error instanceof Error ? error.message : ''; setNotice(message.includes('destination') ? 'Сначала подтвердите замену папки destination' : message || 'Не удалось поставить трек в очередь') } finally { setReprocessing(false) } }
  ...
  useEffect(() => { const parent = document.querySelector('.sidebar-bottom'); if (!parent) return; const button = document.createElement('button'); button.className = 'nav-item quiet recovery-library-action'; button.type = 'button'; button.textContent = 'Восстановить библиотеку'; button.disabled = reprocessing || scanning; button.addEventListener('click', () => void scan()); parent.append(button); return () => button.remove() }, [reprocessing, scanning])
...
function TrackDetail({ detail, sourceId, layer, setLayer, tags, draft, setDraft, saving, reprocessing, onSave, onRetry }: { readonly detail: Detail | null; readonly sourceId: string; readonly layer: Layer; readonly setLayer: (value: Layer) => void; readonly tags: Tags; readonly draft: Tags; readonly setDraft: (value: Tags) => void; readonly saving: boolean; readonly reprocessing: boolean; readonly onSave: () => void; readonly onRetry: () => void }) {
  ...
  useEffect(() => { if (!detail?.destination_conflict) return; const banner = document.querySelector('.workflow-banner'); if (!banner) return; const button = document.createElement('button'); button.className = 'secondary destination-replace-action'; button.type = 'button'; button.textContent = 'Заменить destination и восстановить'; button.disabled = reprocessing; button.addEventListener('click', () => { button.disabled = true; void api<{ queued: boolean }>(`/api/library/records/${detail.record_id}/sources/${sourceId}/destination-conflict/replace`, { method: 'POST' }).then(() => window.location.reload()).catch(() => { button.disabled = false }) }); banner.append(button); return () => button.remove() }, [detail, reprocessing, sourceId])
