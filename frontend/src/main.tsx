...
type LibraryRecordView = { record_id: string; title: string; incoming_folder: string; publication: { state: string }; attention: boolean; tracks: Track[]; audit: Array<{ action: string; actor: string; details: Record<string, unknown> }> }
type QueueItem = { record_id: string; title: string; artist: string; album: string; incoming_folder: string; publication_state: string; attention: boolean }
type LibraryRecordSummary = { record_id: string; source_state: string; processing_state: string; match_state: string; publication_state: string; sources: Array<{ path: string }> }
type LibraryRecordDetail = { record_id: string; states: { source: string; processing: string; match: string; publication: string; metadata: string }; sources: Array<{ source_id: string; path: string; state: string }>; publications: Array<{ state: string }>; metadata_revisions: Array<{ source_id: string; layer: string; revision: number; tags: Tags; actor: string }>; events: Array<{ kind: string; state: string; reason: string | null; details: Record<string, unknown>; created_at: string }> }
...
function recordView(record: LibraryRecordDetail): LibraryRecordView {
  ...
  return {
    ...
    attention: record.states.match !== 'matched' || record.states.publication !== 'published',
...
function App() {
  ...
  async function loadQueue() {
    ...
    try {
      ...
      const items = payload.items.map((record) => ({
        record_id: record.record_id,
        title: recordTitle(record),
        artist: record.match_state,
        album: record.publication_state,
        incoming_folder: record.sources[0]?.path || 'Источник не указан',
        publication_state: record.publication_state,
        attention: record.match_state !== 'matched' || record.publication_state !== 'published',
      }))
      setQueue(items)
  ...
  const visibleQueue = useMemo(() => queue.filter((item) => {
    ...
    return haystack.includes(query.toLowerCase()) && (view === 'library' || item.attention)
  }), [queue, query, view])
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="nav" aria-label="Основная навигация">
        ...
        <button className={view === 'attention' ? 'nav-item active' : 'nav-item'} onClick={() => setView('attention')}><Icon name="settings" /> Требует внимания <span className="nav-alert">{queue.filter((item) => item.attention).length}</span></button>
      </nav>
    ...
    <main className="workspace">
      ...
      <div className="content">
        <section className="hero"><div><p className="eyebrow">{view === 'library' ? 'Ваша коллекция' : 'Нужна проверка'}</p><h1>{view === 'library' ? 'Медиатека' : 'Проверьте эти релизы'}</h1><p className="hero-copy">{view === 'library' ? 'Все релизы, треки и теги в одном спокойном рабочем пространстве.' : 'Низкая уверенность и неоднозначные совпадения собраны здесь.'}</p></div><div className="hero-stat"><strong>{queue.length.toString().padStart(2, '0')}</strong><span>релизов<br />в каталоге</span></div></section>
         ...
         <div className="library-grid"><section className="release-panel"><div className="section-heading"><div><p className="eyebrow">{visibleQueue.length} результатов</p><h2>Каталог записей</h2></div><div className="section-actions"><button className="refresh" disabled={scanLoading} onClick={() => void scanIncoming()}>{scanLoading ? 'Сканирование…' : 'Сканировать файлы'}</button><button className="refresh" onClick={() => void loadQueue()}>Обновить</button></div></div>{loading ? <div className="empty-state">Загрузка каталога…</div> : groups.length ? groups.map(([key, items]) => <div className="group" key={key}><div className="group-title"><Icon name={group === 'folder' ? 'folder' : group === 'album' ? 'disc' : 'music'} /><span>{key}</span><small>{items.length}</small></div>{items.map((item) => <button className={item.record_id === selectedReleaseId ? 'release-row selected' : 'release-row'} key={item.record_id} onClick={() => setSelectedReleaseId(item.record_id)}><span className="cover"><Icon name="disc" /></span><span className="release-copy"><strong>{item.title || item.record_id}</strong><small>{item.artist || item.incoming_folder}</small></span><span className={item.attention ? 'badge warning' : 'badge'}>{item.attention ? 'Проверить' : 'Готов'}</span><Icon name="chevron" /></button>)}</div>) : <div className="empty-state"><Icon name="library" /><strong>Записи не найдены</strong><span>Измените запрос или дождитесь публикации новых файлов.</span></div>}</section>
