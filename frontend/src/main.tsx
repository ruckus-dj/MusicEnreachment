...
function tagsFor(item: Summary, sourceId: string, layer: Layer): Tags { return item.metadata_revisions?.find((revision) => revision.source_id === sourceId && revision.layer === layer)?.tags ?? {} }
...
function TrackDetail({ detail, sourceId, layer, setLayer, tags, draft, setDraft, saving, onSave }: { readonly detail: Detail | null; readonly sourceId: string; readonly layer: Layer; readonly setLayer: (value: Layer) => void; readonly tags: Tags; readonly draft: Tags; readonly setDraft: (value: Tags) => void; readonly saving: boolean; readonly onSave: () => void }) {
  ...
  return <section className="inspector"><div className="inspector-grid"><div><div className="file-card"><span className="entity-art disc">◉</span><div><p className="eyebrow">Исходный файл</p><strong>{source.path.split('/').at(-1)}</strong><small>{format} · {source.size_bytes ? `${Math.round(source.size_bytes / 1024)} KB` : 'размер неизвестен'} · {source.state}</small></div></div><div className="evidence-card"><div className="section-heading"><div><p className="eyebrow">Анализ файла</p><h2>Что найдено</h2></div><span className="badge success">{detail.states.match === 'matched' ? 'Совпадение' : 'Нужна проверка'}</span></div><div className="evidence-grid"><div><span>Fingerprint</span><strong>{fingerprint?.fingerprint ? 'Сформирован' : 'Не найден'}</strong><small>{fingerprint?.tool_version ?? 'инструмент не указан'}</small></div><div><span>AcousticID</span><strong>{attempts.find((attempt) => attempt.provider === 'acoustid')?.outcome ?? 'Не запускался'}</strong><small>{attempts.length ? `попыток: ${attempts.length}` : 'после сканирования появится здесь'}</small></div></div><p className="hash">SHA-256: {source.sha256}</p></div><div className="history-card"><p className="eyebrow">История</p>{detail.events.length ? detail.events.slice().reverse().map((event) => <div className="history-line" key={`${event.kind}-${event.created_at}`}><span /><div><strong>{event.kind.replaceAll('_', ' ')}</strong><small>{new Date(event.created_at).toLocaleString('ru-RU')} · {event.reason ?? event.state}</small></div></div>) : <p className="muted">История появится после обработки файла.</p>}</div></div><div className="metadata-card"><div className="section-heading"><div><p className="eyebrow">Метаданные</p><h2>Сравнение слоёв</h2></div><span className="revision">rev {detail.metadata_revisions?.filter((item) => item.source_id === sourceId && item.layer === 'final').at(-1)?.revision ?? 0}</span></div><div className="layer-tabs">{(['original', 'analyzed', 'final'] as const).map((value) => <button className={layer === value ? 'active' : ''} key={value} onClick={() => setLayer(value)}>{value === 'original' ? 'Исходные' : value === 'analyzed' ? 'Анализ' : 'Финальные'}</button>)}</div>{layer !== 'final' && <div className="read-only-note">Слой зафиксирован при сканировании и не редактируется. Переключитесь на «Финальные», чтобы внести правку.</div>}<div className="tag-grid">{TAG_FIELDS.map((field) => <label key={field}><span>{field}</span><input value={tags[field] ?? ''} readOnly={layer !== 'final'} onChange={(event) => setDraft({ ...draft, [field]: event.target.value })} /></label>)}</div><div className="editor-footer"><span className="muted">Исходные и анализ остаются неизменяемыми.</span><button className="primary" disabled={saving} onClick={onSave}>{saving ? 'Сохраняем…' : 'Сохранить финальные'}</button></div></div></div></section>
}
