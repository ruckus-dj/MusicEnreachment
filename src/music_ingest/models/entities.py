...
class SourceRecord(Base):
    ...
    source_metadata_revision: Mapped[int] = mapped_column(Integer, nullable=False, default=1, server_default='1')
...
class SourceTagRecord(Base):
    ...
    prepared_bytes: Mapped[bytes | None] = mapped_column(LargeBinary)
    binary_evidence: Mapped[bytes | None] = mapped_column(LargeBinary)
    physical_id: Mapped[str | None] = mapped_column(Text)
    extraction_version: Mapped[str | None] = mapped_column(Text)
    selected: Mapped[bool] = mapped_column(nullable=False, default=True, server_default='true')
    declared_codec: Mapped[str | None] = mapped_column(Text)
    applied_choice_json: Mapped[str | None] = mapped_column(Text)
    decision_origin: Mapped[str | None] = mapped_column(Text)
